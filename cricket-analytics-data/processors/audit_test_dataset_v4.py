                ...
                for d in over.get("deliveries", []) or []:
                    ...
                    if wickets:
                        for w in wickets:
                            ...
                            # Player-match wicket totals are bowler-credit wickets.
                            if bowler_ext and kind not in BOWLER_NOT_CREDITED:
                                raw_pm[(mid, bowler_ext)]["wickets"] += 1
...
section("10. MAIDEN-OVER SANITY CHECK")
print("Maiden values are retained in player-match CSVs.")
print("They are NOT compared at career level because player_statistics.csv has no maidens column.")
print("The exact maiden algorithm will be audited separately if needed.")
print("[PASS] Maiden values are not used as a career reconciliation gate.")
...
# Inspect the actual CSV schema instead of assuming a field name.
match_header_path = f"{PROCESSED}\\matches.csv"
with open(match_header_path, "r", encoding="utf-8-sig", newline="") as f:
    reader = csv.DictReader(f)
    actual_headers = reader.fieldnames or []

print("matches.csv fields:", ", ".join(actual_headers))

status_field = next(
    (x for x in ["match_status", "status", "result_type", "result"] if x in actual_headers),
    None
)

status_counts = defaultdict(int)
result_type_counts = defaultdict(int)
result_desc_counts = defaultdict(int)

for r in matches_p.values():
    if status_field:
        status_counts[str(r.get(status_field, "")).strip()] += 1
    if "result_type" in actual_headers:
        result_type_counts[str(r.get("result_type", "")).strip()] += 1
    if "result_description" in actual_headers:
        result_desc_counts[str(r.get("result_description", "")).strip()] += 1
...
if result_type_counts:
    print("Result-type counts:", dict(result_type_counts))

if result_desc_counts:
    print("Result-description sample/counts:")
    for k, v in list(result_desc_counts.items())[:20]:
        print(" ", repr(k), ":", v)

# The raw Test dataset was independently audited as:
# 739 winner outcomes + 179 draws = 918.
raw_draws = 0
raw_winners = 0
raw_other = []

for mid, data in raw_matches.items():
    ...
    if outcome.get("result") == "draw":
        ...
        raw_other.append(mid)

print("Raw draw matches:", raw_draws)
print("Raw winner matches:", raw_winners)
print("Raw other/unclassified outcomes:", len(raw_other))
if raw_other:
    print("Other examples:", raw_other[:20])

result(
    raw_draws == 179 and raw_winners == 739 and not raw_other,
    "Raw Test outcomes reconcile to 179 draws + 739 winner results = 918."
)

# Also verify the processed file has the same match-level result type where
# the schema exposes it.
if "result_type" in actual_headers:
    processed_result_types = {r["result_type"].strip() for r in matches_p.values()}
    print("Processed result types:", sorted(processed_result_types))
