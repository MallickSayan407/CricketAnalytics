# =============================================================================
# CRICKET ANALYTICS - TEST DATA PROCESSOR
# CORRECTED VERSION
#
# Confirmed fixes:
# 1. No-balls do NOT count as legal balls.
# 2. Career runs_conceded is accumulated independently of balls_bowled.
# 3. timed out is NOT credited as a bowler wicket.
# 4. Test matches process actual innings/deliveries; no fixed four-innings
#    or overs assumption is made.
# 5. Declarations and innings-level penalty runs are preserved.
# 6. Retired hurt/not out are not treated as batting dismissals.
# =============================================================================
...
ZIP_PATH = Path(
    r"D:\CricketAnalytics\cricket-analytics-data\raw\international\tests_json.zip"
)

OUTPUT_DIR = Path(
    r"D:\CricketAnalytics\cricket-analytics-data\processed\test"
)
...
def main():
    ...
    with zipfile.ZipFile(ZIP_PATH, "r") as z:
        ...
        for index, member in enumerate(
            ...
            # Test matches do not have a fixed overs limit.
            # The database field is retained for compatibility, so use
            # 0 rather than inventing an overs value.
            overs_value = 0
            ...
            for innings_data in innings:
                ...
                # -------------------------------------------------
                # Innings-level penalty runs
                #
                # Cricsheet stores these outside individual deliveries.
                # They are part of the batting team's total but are not
                # batter runs and are not bowler-conceded runs.
                # -------------------------------------------------
                penalty_data = innings_data.get("penalty_runs", {})
                if not is_super_over and isinstance(penalty_data, dict):
                    penalty_value = sum(
                        safe_int(value)
                        for value in penalty_data.values()
                    )
                    if penalty_value:
                        team_stats[batting_team]["runs"] += penalty_value
                        team_stats[batting_team]["extras"] += penalty_value
                        regulation_runs += penalty_value
