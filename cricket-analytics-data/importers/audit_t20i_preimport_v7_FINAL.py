"""
CRICKET ANALYTICS
T20I PRE-IMPORT AUDIT V7 - FINAL
======================
...
with zipfile.ZipFile(ZIP_PATH, "r") as zf:
    ...
    for index, filename in enumerate(json_files, start=1):
        ...
        for team in info.get("teams", []):
            ...
            if str(team).strip():
                raw_match_team_keys.add((match_id, norm(team)))
...
section("2. MATCH-TEAM RAW -> PROCESSED RECONCILIATION (INCLUDING ZERO-BALL INNINGS)")
...
raw_team_keys = set(raw_match_team_keys)
...
for (mid, eid), row in processed_pm.items():
    ...
    # Bowling innings / balls / wickets are counted only when a legal ball
    # was bowled, matching the processor.
    if balls_bowled > 0:
        d["bowling_innings"] += 1
    ...
    # Runs conceded can be non-zero even when no legal ball was bowled
    # (e.g. a wide/no-ball). Accumulate independently.
...
if FAIL == 0:
    print("T20I PRE-IMPORT AUDIT V7 - FINAL: PASS")
    print()
    print(
    ...
    print("T20I PRE-IMPORT AUDIT V7 - FINAL: FAIL")
    print()
    print(
